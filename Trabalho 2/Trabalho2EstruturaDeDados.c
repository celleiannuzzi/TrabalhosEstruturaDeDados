...
// Trabalho 2 - estrutura de dados - Marcelle de Macedo Iannuzzi Barbosa - DRE 118056472
...
void insereVertice(tno * ptlista,int numVertice){ 
...
void insereAdj(tno * ptlista, int numLido, int index){
...
tno * retornaVertice(tno * ptlista, int index){
    ...
    return retornaVertice(ptlista->prox, index);
}
...
void imprimeAdj(tno *ptlista){
    tno *p;
    for (p = ptlista->prox; p != NULL; p = p->prox){
        for(int i = 0; i < p->numAdj; i++)
...
int main(){
    ...
    while(i < pos){ // enquanto o aux i for menor que o aux pos
        p = retornaVertice(ptlista, ordem[i]);
        //printf("loop: %d - vertice atual: %d\n", i,  p->numVertice);
        for(j = 0; j < p->numAdj; j++){
            aux = p->adj[j];
